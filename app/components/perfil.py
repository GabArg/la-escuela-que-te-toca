...
def render_profile(
    ...
    if history is not None:
        ...
        render_history(profiles, history, territory_id, embedded=True)

    st.markdown('<div class="section-rule"></div>', unsafe_allow_html=True)
    st.subheader("¿Qué muestran el acceso y la asistencia?")
    ...
    with st.expander("Comparar asistencia 15–17 con medianas"):
        ...
        if figure is not None: st.plotly_chart(figure, use_container_width=True)
    ...
    st.markdown('<div class="section-rule"></div>', unsafe_allow_html=True)
    st.subheader("¿Cómo está desplegada la oferta educativa?")
    st.caption("Una localización corresponde a una sede o anexo (CUE-anexo). CUE organiza establecimientos institucionales; no equivale necesariamente a edificio.")
    definitions = [
    ...
    st.markdown('<div class="section-rule"></div>', unsafe_allow_html=True)
    st.subheader("¿Qué condiciones rodean estos resultados?")
    ...
    st.markdown('<div class="section-rule"></div>', unsafe_allow_html=True)
    st.subheader("¿Qué muestran los aprendizajes?")
    st.caption("Aprender Secundaria 2024 · operativo censal. Censal describe el diseño, no participación completa.")
